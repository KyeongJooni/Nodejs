...
module.exports = {
    ...
    create: (req, res) => {
            db.query('select * from topic', (err, topics) => {
                ...
                db.query('select * from author', (err2, authors) => {
                    ...
                    var context = { title: 'WEB AUTHOR 테이블',
                                    list: topics,
                                    menu: tag,
                                    body: b }
    ...
    create_process : (req,res)=>{
        ...
        update: (req, res) => {
            ...
            db.query('SELECT * FROM author WHERE id = ?', [authorId], (error, author) => {
                if (error) {
                    console.error("DB Query Error: ", error);  // DB 에러 로그
                    throw error;
                }
    ...
    // update_process 경로: 저자 정보 업데이트 처리
        ...
        req.on('end', () => {
            ...
            db.query('UPDATE author SET name = ?, profile = ? WHERE id = ?', 
                [sanitizedName, sanitizedProfile, post.id], (error, result) => {
                    ...
                    res.redirect(`/author`);  // 업데이트 후 저자 목록 페이지로 리다이렉트
                    res.end();
                });
        });
    },
    
    // 저자 삭제 처리: 바로 삭제되도록 수정
delete: (req, res) => {
    var authorId = req.params.authorId;  // URL에서 authorId 가져오기
